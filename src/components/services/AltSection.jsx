...
// Visual panel — image instead of emoji
function Visual({ image, alt = '', dark }) {
  return (
    <div
      className={`
        relative h-[360px] rounded-[20px] flex items-center justify-center
        overflow-hidden border border-DEFAULT shadow-card
        ${dark ? 'bg-ink2' : 'bg-bg'}
      `}
    >
  ...
  return (
    <section className={`py-[80px] px-[5%] ${dark ? 'bg-bg' : 'bg-card'}`}>
      ...
      >
        {/* Visual */}
        <FadeIn className={reverse ? '[direction:ltr]' : ''}>
          <Visual image={image} alt={imageAlt} dark={dark} />
        </FadeIn>
